...
import { Game, Timeline } from "@/types/custom.types";
...
export default function GameDetail({ game }: { game: Game }) {
    const [feed, setFeed] = useState<Timeline>([]);
    const [feedLoading, setFeedLoading] = useState(true);
    const router = useRouter();

    useEffect(() => {
        const db = new BrowserDatabaseClient();
        db.getTimeline({ gameId: game.id }).then((feed) => {
            setFeed(feed);
            console.log(feed);
    ...
    return (
        <div className="shadow overflow-hidden rounded-lg dark:shadow-md dark:bg-slate-800 border-gray-200 dark:border-gray-700 border">
            ...
            <div className="p-6">
                <GameFeed feed={feed} isLoading={feedLoading} />
            </div>
        </div>
    );
}
